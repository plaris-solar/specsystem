   ...
   <q-card class="dialog-window">
        ...
        <q-card-section class="q-pt-none row">
            <q-table
                ...
                data-cy="spec-table">
                <template v-slot:top>
                    Role Assigned Pending Signatures
                </template>
                <template v-slot:header="props">
                    <q-th v-for="col in props.cols" 
                          :key="col.name" 
        ...
        <q-card-section class="q-pt-none row">
            <q-btn  v-show="isAdmin || props.username === username"
...
<script setup>    
    ...
    async function loadData() {
        ...
        delegates_for.value = res['delegates_for'].map(function(user){ return {user:user}})
        watches.value = res['watches'].map(function(num){ return {num:num}})
